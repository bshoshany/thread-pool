...
namespace BS
{
	...
	public:
		...
		/**
		 * @brief Push a function with zero or more arguments, but no return value, into the task queue if running + queued < #threads, otherwise invoke the function itself. Does not return a future, so the user must use wait_for_tasks() or some other method to ensure that the task finishes executing, otherwise bad things will happen.
		 *
		 * @tparam F The type of the function.
		 * @tparam A The types of the arguments.
		 * @param task The function to push.
		 ...
		 */
		template <typename F, typename... A>
		void push_task_if_available_else_invoke(F&& task, A&&... args)
		{
			tasks_mutex.lock();
			if (tasks_running + tasks.size() < thread_count)
			{
			...
			tasks_mutex.unlock();
			std::invoke(std::bind(std::forward<F>(task), std::forward<A>(args)...));
		}
		...
		/**
		 * @brief Submit a function with zero or more arguments into the task queue if running + queued < #threads, otherwise invoke the function itself. If the function has a return value, get a future for the eventual returned value. If the function has no return value, get an std::future<void> which can be used to wait until the task finishes.
		 *
		 * @tparam F The type of the function.
		 * @tparam A The types of the zero or more arguments to pass to the function.
		 * @tparam R The return type of the function (can be void).
		 ...
		 */
		template <typename F, typename... A, typename R = std::invoke_result_t<std::decay_t<F>, std::decay_t<A>...>>
		[[nodiscard]] std::future<R> submit_if_available_else_invoke(F&& task, A&&... args)//TODO test
		{
			std::shared_ptr<std::promise<R>> task_promise = std::make_shared<std::promise<R>>();
			tasks_mutex.lock();
			if (tasks_running + tasks.size() < thread_count)
